	...
	<div>
		<div class="mb-4 sm:mb-6">
			<button
				...
				class="mb-3 sm:mb-4 inline-flex items-center px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors duration-200"
			>
			...
			<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100 dark:border-gray-700">
				<div class="flex items-center justify-between">
					<div class="flex-1 min-w-0">
						<h2 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-2 break-words">{{ tournament?.name }}</h2>
		...
		<div v-if="tournament && tournament.prize_pool" class="mb-4 sm:mb-6">
			<div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100 dark:border-gray-700">
				<div class="flex items-center justify-between">
					<div>
						<h3 class="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mb-1">Призовой фонд</h3>
		...
		<div v-if="!loading && tournament" class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 sm:p-6 border border-gray-100 dark:border-gray-700">
			<div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3 sm:gap-0">
				<h3 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">Участники</h3>
				<div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
					...
					<button
						...
						class="px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-white bg-gradient-to-r from-green-600 to-emerald-600 rounded-lg hover:from-green-700 hover:to-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto"
		...
		<div v-if="showFinishModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
			<div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
				<div class="p-4 sm:p-6">
					<h3 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4">Завершение турнира</h3>
					...
					<div v-if="tournament && dynamicPrizeDistribution && dynamicPrizeDistribution.length > 0" class="mb-4 sm:mb-6">
						<div class="grid gap-2 sm:gap-4 mb-4 sm:mb-6 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
							<div
								v-for="(prize, index) in dynamicPrizeDistribution"
								:key="prize.place"
								class="p-2 sm:p-4 rounded-lg text-center"
							...
							>
								<div class="text-base sm:text-xl font-bold mb-1 sm:mb-2"
									:class="index === tournament.prize_distribution.length - 1
										? 'text-yellow-600 dark:text-yellow-400' 
										: index === tournament.prize_distribution.length - 2
										? 'text-gray-600 dark:text-gray-400' 
								...
								<div v-if="selectedWinners.length > 0 && getWinnerPlace(selectedWinners[selectedWinners.length - index - 1]) === prize.place" class="mt-1 sm:mt-2 text-xs text-green-600 dark:text-green-400">
									✓ Выбран
								</div>
							</div>
						</div>
						...
						<div class="space-y-2">
							...
							<div
								...
								class="flex items-center justify-between p-3 sm:p-4 border-2 rounded-lg cursor-pointer transition-colors"
							...
							>
								...
								<div v-if="selectedWinners.includes(participant.id)" class="flex items-center space-x-1 sm:space-x-2 flex-shrink-0 ml-2">
									...
									<svg class="w-4 h-4 sm:w-5 sm:h-5 text-indigo-600 dark:text-indigo-400" fill="currentColor" viewBox="0 0 20 20">
					...
					<div class="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-4 sm:mt-0">
						<button
							...
							class="flex-1 px-3 sm:px-4 py-2 text-xs sm:text-sm bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-colors font-medium"
						>
							Завершить турнир
						</button>
						<button
							@click="closeFinishModal"
							class="px-3 sm:px-4 py-2 text-xs sm:text-sm bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors font-medium w-full sm:w-auto"
						>
							Отмена
						</button>
					</div>
		...
		<div v-if="showPrizeDistributionModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4">
			<div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
				<div class="p-4 sm:p-6">
					<h3 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-3 sm:mb-4">Настройка распределения призов</h3>
					<p class="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-4 sm:mb-6">
					...
					<div class="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
						<div
							...
							:key="index"
							class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 p-3 sm:p-4 border border-gray-300 dark:border-gray-600 rounded-lg"
						>
							<div class="w-full sm:w-20">
								...
								<input
									...
									type="number"
									min="1"
									class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-white transition duration-200 text-xs sm:text-sm"
									readonly
								/>
							</div>
							<div class="flex-1">
							...
							<div class="w-full sm:w-32">
								...
								<div class="px-3 py-2 text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-lg">
									{{ formatCurrency((tournament?.prize_pool || 0) * ((prize.percentage || 0) / 100), tournament?.currency) }}
								</div>
							</div>
							<button
								@click="removePrizePlace(index)"
								class="px-3 py-2 text-xs sm:text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors sm:mt-6"
							>
								Удалить
							</button>
						</div>
					...
					<div class="mb-4 sm:mb-6 flex flex-wrap items-center gap-2">
						...
						<button
							...
							class="px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
						>
						...
						<button
							...
							class="px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-colors"
						>
							Очистить кастомное
						</button>
					</div>
					...
					<div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
						<button
							...
							class="flex-1 px-3 sm:px-4 py-2 text-xs sm:text-sm bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-colors font-medium"
						>
							Сохранить
						</button>
						<button
		...
		if (placesCount <= 10 && distributions[placesCount]) {
			percentage = distributions[placesCount][place - 1] || 0;
				...
				if (place === 1) {
					...
					const remainingPlaces = placesCount - 10;
					const remainingPercentage = 100 - 40 - 19 - 13 - 10 - 6 - 4 - 3 - 2 - 2 - 1;
					percentage = Math.round((remainingPercentage / remainingPlaces) * 100) / 100;
				}
			}
	...
	for (let place = 1; place <= itmPlaces; place++) {
		...
		if (itmPlaces <= 10 && distributions[itmPlaces]) {
			...
			if (place === 1) {
				...
				const remainingPlaces = itmPlaces - 10;
				const remainingPercentage = 100 - 40 - 19 - 13 - 10 - 6 - 4 - 3 - 2 - 2 - 1;
				percentage = Math.round((remainingPercentage / remainingPlaces) * 100) / 100;
			}
		}
